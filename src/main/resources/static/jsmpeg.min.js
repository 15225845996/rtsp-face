...
JSMpeg.Decoder.MP2Audio = function() {
    ...
    MP2.prototype.decode = function() {
        console.log("MP2.prototype.decode")
        var startTime = JSMpeg.Now();
...
JSMpeg.Renderer.WebGL = function() {
    ...
    WebGLRenderer.IsSupported = function() {
        try {
            ...
            //TODO 执行以下两行代码， 要不然后续获取canvas toDataURL异常
            var aa = canvas.getContext("2d")
            var bb = canvas.toDataURL("image/jpeg");
    ...
    CanvasRenderer.prototype.render = function(y, cb, cr) {
        ...
        if(num > 3) {
            ...
            this.context.putImageData(this.imageData, 0, 0)
                ...
                faces = getFaces(this.imageData);
                /*faces = getFaces2(this.imageData)*/

            if(faces != null && faces.length > 0){
                for (let i = 0; i < faces.length; i++) {
                    let face = faces[i];
                    //在圈人脸之前截图
                    syncFace(face,this.imageData.width,this.imageData.height)

                    /*圈出人脸*/
                    this.context.strokeStyle="#f60";
                    this.context.lineWidth=4;
                    this.context.strokeRect(face['x'],face['y'],face['w'],face['h']);
                }
            }
        }
    };
